...
- `pyproject.toml` — pin `parsimony-core>=0.0.2` (or `parsimony-core[catalog]>=0.0.2` for catalog-backed packages), declare a
...
- `scripts/build_catalog.py` *(only if maintainers build a hosted
  ...
  `list(result.entities.values())`, builds the index policy with `discovery_indexes`,
  calls `catalog.build()`, then `catalog.save(...)` for local paths or `hf://...` uploads.
  (Broad search targets the `title` index by convention — there is no `default_field` to set.)
