...
1. Spawns the parsimony-mcp server in-process with discover.load_all()
...
Usage (from the parsimony-connectors workspace root):
    uv run python scripts/mcp_e2e_test.py
"""
...
def _parse_toon_search_result(text: str) -> list[dict[str, str]]:
    ...
    header_idx = next((i for i, line in enumerate(lines) if "{code" in line and "}" in line), None)
...
def _treasury_dispatch(code: str) -> FetchSpec:
    ...
    return FetchSpec(
        "treasury_fetch",
        lambda _c: {"endpoint": endpoint, "filter": "record_date:gte:2024-01-01", "page_size": 50},
    )
...
async def main() -> int:
    print("=== Spawning MCP server with discover.load_all() ===", flush=True)
    ...
    for provider, cfg in PROVIDERS.items():
        ...
        for q in res["queries"]:
            if q["ok"]:
                ...
                print(
                    f"  PASS  {q['query']!r}\n"
                    f"        top: {q['top_code']:<40} score={q['top_score']}  ({q['search_ms']}ms search)\n"
