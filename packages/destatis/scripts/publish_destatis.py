"""Publish parsimony-destatis catalog snapshots.
...
(``~/.cache/parsimony/catalogs/destatis/`` on Linux). Override with
``PARSIMONY_CACHE_DIR`` if you need a different cache root (HF runners,
alternate disks).

Run with ``uv run --extra publish python scripts/publish_destatis.py`` from
the package root. After publishing, push to HF::

    hf upload ockham/destatis "$(parsimony cache info | grep catalogs)/destatis/"
"""
...
TARGET_ROOT = catalogs_dir("destatis")


async def _main() -> int:
    print(f"=== publish destatis start {time.strftime('%Y-%m-%dT%H:%M:%S%z')} ===", flush=True)
