...
def build(*, save: str | None, push: str | None) -> None:
    catalog = build_destatis_catalog()
    logger.info("Built %s catalog with %d entries", catalog.name, len(catalog))
    if save is not None:
        catalog.save(save, builder="packages/destatis/scripts/build_catalog.py")
    if push is not None:
        catalog.save(push, builder="packages/destatis/scripts/build_catalog.py")
