...
def test_treasury_rates_fetch_parses_xml_and_normalises_record_date() -> None:
    ...
    # ``source_url`` is declared METADATA so consumers don't read it as a data
    # measure; the spec annotates only declared columns, so the feed-specific
    # rate columns (``BC_10YEAR``) stay out of it.
