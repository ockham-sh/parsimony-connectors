"""Publish parsimony-treasury catalog snapshots.
...
(``~/.cache/parsimony/catalogs/treasury/`` on Linux). Override with
``PARSIMONY_CACHE_DIR`` if you need a different cache root (HF runners,
alternate disks).

Run with ``uv run --extra publish python scripts/publish_treasury.py`` from
the package root. After publishing, push to HF::

    hf upload ockham/treasury "$(parsimony cache info | grep catalogs)/treasury/"
"""
...
TARGET_ROOT = catalogs_dir("treasury")


async def _main() -> int:
    print(f"=== publish treasury start {time.strftime('%Y-%m-%dT%H:%M:%S%z')} ===", flush=True)
