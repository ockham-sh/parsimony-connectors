...
| `treasury_search` | connector | Keyword-search the published `treasury` catalog (`query=` is literal text; exact constraints via `filter=`). Returns `code` + `title` + METADATA. Dispatch from METADATA: `source=fiscal_data` → `treasury_fetch(endpoint=...)`; `source=treasury_rates` → `treasury_rates_fetch`. Do not parse the compound `code`. |
