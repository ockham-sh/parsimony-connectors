...
- **Requires `parsimony-core>=0.3,<0.5`.** Aligns with the kernel flat-module
  refactor: `parsimony.transport.http` moved to `parsimony.http`, and the
  `Namespace("x")` annotation class was replaced by the string sentinel
  `Annotated[str, "ns:x"]`. Both changes are mechanical but user-visible if
  you had imported the old symbols transitively.
...
- ``HttpClient`` imported from ``parsimony.http`` (was
