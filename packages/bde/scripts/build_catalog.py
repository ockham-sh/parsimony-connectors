...
from parsimony_bde import BdeEnumerateParams, enumerate_bde
...
async def build(*, save: str | None, push: str | None) -> Catalog:
    ...
    if save is not None:
        await catalog.save(save, builder="packages/bde/scripts/build_catalog.py")
    if push is not None:
        await catalog.save(push, builder="packages/bde/scripts/build_catalog.py")
    return catalog
...
def main() -> None:
    ...
    parser.add_argument("--push", help="Catalog URL to push, e.g. hf://parsimony-dev/bde.")
