"""Publish parsimony-bde catalog snapshots.
...
(``~/.cache/parsimony/catalogs/bde/`` on Linux). Override with
``PARSIMONY_CACHE_DIR`` if you need a different cache root (HF runners,
alternate disks).

Run with ``uv run --extra publish python scripts/publish_bde.py`` from
the package root. After publishing, push to HF::

    hf upload ockham/bde "$(parsimony cache info | grep catalogs)/bde/"
"""
...
TARGET_ROOT = catalogs_dir("bde")


async def _main() -> int:
    print(f"=== publish bde start {time.strftime('%Y-%m-%dT%H:%M:%S%z')} ===", flush=True)
