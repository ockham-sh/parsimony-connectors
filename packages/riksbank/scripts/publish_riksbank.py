"""Publish parsimony-riksbank catalog snapshots.
...
(``~/.cache/parsimony/catalogs/riksbank/`` on Linux). Override with
``PARSIMONY_CACHE_DIR`` if you need a different cache root (HF runners,
alternate disks).

Run with ``uv run --extra publish python scripts/publish_riksbank.py`` from
the package root. After publishing, push to HF::

    hf upload ockham/riksbank "$(parsimony cache info | grep catalogs)/riksbank/"
"""
...
TARGET_ROOT = catalogs_dir("riksbank")


async def _main() -> int:
    print(f"=== publish riksbank start {time.strftime('%Y-%m-%dT%H:%M:%S%z')} ===", flush=True)
