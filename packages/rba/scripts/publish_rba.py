"""Publish parsimony-rba catalog snapshots.
...
(``~/.cache/parsimony/catalogs/rba/`` on Linux). Override with
``PARSIMONY_CACHE_DIR`` if you need a different cache root (HF runners,
alternate disks).

Run with ``uv run --extra publish python scripts/publish_rba.py`` from
the package root. After publishing, push to HF::

    hf upload ockham/rba "$(parsimony cache info | grep catalogs)/rba/"
"""
...
TARGET_ROOT = catalogs_dir("rba")


async def _main() -> int:
    print(f"=== publish rba start {time.strftime('%Y-%m-%dT%H:%M:%S%z')} ===", flush=True)
