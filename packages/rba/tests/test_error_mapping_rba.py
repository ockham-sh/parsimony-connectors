...
class _ErrSession:
    ...
    def __init__(self, *, status: int | None = None, raises: BaseException | None = None,
...
@pytest.mark.asyncio
async def test_curl_get_maps_status_to_typed_error(status: int, exc_type: type) -> None:
    session = _ErrSession(status=status)
    with pytest.raises(exc_type):
        await pkg._curl_get(cast(Any, session), "https://www.rba.gov.au/x", op_name="test")


@pytest.mark.asyncio
async def test_curl_get_provider_error_carries_status_code() -> None:
...
@pytest.mark.asyncio
async def test_curl_get_rate_limit_carries_retry_after() -> None:
...
@pytest.mark.asyncio
async def test_curl_get_rate_limit_defaults_retry_after_without_header() -> None:
...
@pytest.mark.asyncio
async def test_curl_get_timeout_maps_to_provider_error_408() -> None:
...
@pytest.mark.asyncio
async def test_curl_get_connection_error_maps_to_provider_error_408() -> None:
...
@pytest.mark.asyncio
async def test_curl_get_returns_text_on_200() -> None:
    class _OkSession:
        async def get(self, url: str, *, impersonate: str = "chrome", timeout: float = 60.0) -> _FakeResponse:
            r = _FakeResponse(200)
            r.text = "hello body"
    ...
    body = await pkg._curl_get(cast(Any, _OkSession()), "https://www.rba.gov.au/x", op_name="test")
    assert body == "hello body"


@pytest.mark.asyncio
async def test_curl_get_returns_bytes_on_200_binary() -> None:
    class _OkSession:
        async def get(self, url: str, *, impersonate: str = "chrome", timeout: float = 60.0) -> _FakeResponse:
            r = _FakeResponse(200)
    ...
    data = await pkg._curl_get(cast(Any, _OkSession()), "https://www.rba.gov.au/x.xlsx", op_name="test", binary=True)
