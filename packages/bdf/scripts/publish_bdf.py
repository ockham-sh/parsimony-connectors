"""Publish parsimony-bdf catalog snapshots.
...
(``~/.cache/parsimony/catalogs/bdf/`` on Linux). Override with
``PARSIMONY_CACHE_DIR`` if you need a different cache root (HF runners,
alternate disks).

Run with ``uv run --extra publish python scripts/publish_bdf.py`` from
the package root. After publishing, push to HF::

    hf upload ockham/bdf "$(parsimony cache info | grep catalogs)/bdf/"
"""
...
TARGET_ROOT = catalogs_dir("bdf")


async def _main() -> int:
    print(f"=== publish bdf start {time.strftime('%Y-%m-%dT%H:%M:%S%z')} ===", flush=True)
