...
async def test_bdf_fetch_parses_json_response() -> None:
    route = respx.get(_OBSERVATIONS_URL).mock(
        return_value=httpx.Response(200, json=_BDF_OBS_JSON)
    )

    result = await bdf_fetch.bind(api_key=_KEY)(key="EXR.M.USD.EUR.SP00.E")
...
@respx.mock
@pytest.mark.asyncio
async def test_bdf_fetch_does_not_leak_secret_into_provenance() -> None:
...
@respx.mock
@pytest.mark.asyncio
async def test_bdf_fetch_applies_period_filters_to_where_clause() -> None:
    route = respx.get(_OBSERVATIONS_URL).mock(
        return_value=httpx.Response(200, json=_BDF_OBS_JSON)
    )

    await bdf_fetch.bind(api_key=_KEY)(
...
@respx.mock
@pytest.mark.asyncio
async def test_bdf_fetch_raises_empty_data_on_empty_array() -> None:
    respx.get(_OBSERVATIONS_URL).mock(return_value=httpx.Response(200, json=[]))

    with pytest.raises(EmptyDataError) as exc:
        await bdf_fetch.bind(api_key=_KEY)(key="XX")
    assert exc.value.query_params == {"key": "XX"}


@respx.mock
@pytest.mark.asyncio
async def test_bdf_fetch_raises_empty_data_when_no_rows_parse() -> None:
    # 200 with rows that all lack a usable date → nothing parses → EmptyData.
    respx.get(_OBSERVATIONS_URL).mock(
        return_value=httpx.Response(200, json=[{"obs_value": 1.0}])
    )

    with pytest.raises(EmptyDataError):
        await bdf_fetch.bind(api_key=_KEY)(key="EXR.M.USD.EUR.SP00.E")


@respx.mock
@pytest.mark.asyncio
async def test_bdf_fetch_raises_parse_error_on_non_list_body() -> None:
...
@pytest.mark.asyncio
async def test_bdf_fetch_rejects_empty_key() -> None:
    with pytest.raises(InvalidParameterError):
        await bdf_fetch.bind(api_key=_KEY)(key="   ")


@pytest.mark.asyncio
async def test_bdf_fetch_no_key_fast_fails_unauthorized(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("BDF_API_KEY", raising=False)
    with pytest.raises(UnauthorizedError) as exc:
        await bdf_fetch(key="EXR.M.USD.EUR.SP00.E", api_key="")
...
@respx.mock
@pytest.mark.asyncio
async def test_bdf_fetch_env_fallback_supplies_key(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("BDF_API_KEY", _KEY)
    route = respx.get(_OBSERVATIONS_URL).mock(
        return_value=httpx.Response(200, json=_BDF_OBS_JSON)
    )

    result = await bdf_fetch(key="EXR.M.USD.EUR.SP00.E")
...
@respx.mock
@pytest.mark.asyncio
async def test_enumerate_bdf_bounded_shape_and_metadata() -> None:
    ...
    result = await enumerate_bdf.bind(api_key=_KEY)()
    df = result.data
...
@respx.mock
@pytest.mark.asyncio
async def test_enumerate_bdf_emits_stub_only_on_series_fetch_failure() -> None:
    respx.get(_DATASETS_URL).mock(return_value=httpx.Response(200, json=_BDF_DATASETS_JSON))
...
@respx.mock
@pytest.mark.asyncio
async def test_enumerate_bdf_empty_catalog_when_dataset_list_fails() -> None:
...
@respx.mock
@pytest.mark.asyncio
async def test_enumerate_bdf_sends_auth_header_and_does_not_leak() -> None:
    ds_route = respx.get(_DATASETS_URL).mock(
        return_value=httpx.Response(200, json=_BDF_DATASETS_JSON)
    )
    respx.get(_SERIES_URL).mock(return_value=httpx.Response(200, json=_BDF_SERIES_JSON))
...
@pytest.mark.asyncio
async def test_enumerate_bdf_no_key_fast_fails_unauthorized(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("BDF_API_KEY", raising=False)
    with pytest.raises(UnauthorizedError) as exc:
        await enumerate_bdf(api_key="")
    assert exc.value.env_var == "BDF_API_KEY"
    assert exc.value.provider == "bdf"


@pytest.mark.asyncio
async def test_enumerate_bdf_seam_is_monkeypatchable(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    async def _fake_datasets(_fetcher: object) -> list[dict[str, object]]:
...
async def _build_fixture_catalog(out_dir: Path) -> None:
...
@pytest.mark.asyncio
async def test_bdf_search_ranks_over_fixture_catalog(tmp_path: Path) -> None:
    out_dir = tmp_path / "bdf_catalog"
    await _build_fixture_catalog(out_dir)
...
@pytest.mark.asyncio
async def test_bdf_search_raises_empty_data_on_no_match(tmp_path: Path) -> None:
    out_dir = tmp_path / "bdf_catalog"
    await _build_fixture_catalog(out_dir)

    with pytest.raises(EmptyDataError):
        await bdf_search(query="zzzzz nonexistent xyzzy plover", limit=5, catalog_url=str(out_dir))
