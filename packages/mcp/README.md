...
**Alpha — `0.1.0a1`.** Public API surface (`create_server`, `connector_to_tool`, `result_to_content`) is stable; behavior-shaping prose strings (error directives, instruction template, truncation footer) are guarded by the test suite and changes require an LLM eval pass.
...
```bash
git clone https://github.com/ockham-sh/parsimony-connectors
cd parsimony-connectors
uv sync                            # workspace install incl. parsimony-mcp + connectors
uv run --package parsimony-mcp pytest packages/mcp/tests --no-cov -q
uv run --package parsimony-mcp ruff check packages/mcp/parsimony_mcp packages/mcp/tests
uv run --package parsimony-mcp mypy packages/mcp/parsimony_mcp
```

Final package size after the simplification: 7 source modules, ~700 LOC, 128 tests.
