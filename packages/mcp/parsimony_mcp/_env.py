...
:func:`parsimony.discovery.build_connectors_from_env` snapshots env
vars at construction time. The returned ``MappingProxyType`` is a
read-only snapshot of the post-load environment for inspection and
testing; it is not the runtime configuration source.
"""
