...
def test_full_text_search_dates_set_daterange_custom() -> None:
    ...
    with patch.dict("os.environ", _UA), patch.object(search_mod, "fetch_json", new=mock):
        sec_edgar_full_text_search(
            query="apple", forms="10-K", start_date="2023-01-01", end_date="2023-12-31"
        )
...
def test_submissions_form_filter() -> None:
    ...
    assert list(result.data["form"]) == ["8-K"]
...
def test_submissions_include_older_walks_pages() -> None:
    ...
    assert "0000999-01-000001" in set(result.data["accessionNumber"])
...
def test_fetch_filing_explicit_document_skips_index() -> None:
    ...
    with patch.dict("os.environ", _UA):
        result = sec_edgar_fetch_filing(
            cik="320193", accession_number="0000320193-24-000123", document="custom.htm"
        )
...
def _concept_payload() -> dict:
    return {
        ...
        "units": {
            "USD": [
                {"end": "2023-09-30", "val": 383285000000, "accn": "a-1", "fy": 2023, "fp": "FY",
                 "form": "10-K", "filed": "2023-11-03", "start": "2022-10-01"},
                {"end": "2022-09-24", "val": 394328000000, "accn": "a-2", "fy": 2022, "fp": "FY",
                 "form": "10-K", "filed": "2022-10-28", "start": "2021-09-26"},
            ]
        },
    }
...
def test_company_concept_unit_filter() -> None:
    ...
    assert set(result.data["unit"]) == {"CAD"}
...
def _frames_payload() -> dict:
    return {
        ...
        "pts": 2,
        "data": [
            {"accn": "a-1", "cik": 1750, "entityName": "AAR CORP.", "loc": "US-IL",
             "end": "2019-02-28", "val": 218600000},
            {"accn": "a-2", "cik": 320193, "entityName": "Apple Inc.", "loc": "US-CA",
             "end": "2019-03-30", "val": 37000000000},
        ],
    }
...
def test_holdings_13f_returns_frame() -> None:
    ...
    df = result.data
    assert list(df.columns) == [
        "cusip", "issuer", "ticker", "security_class", "security_type", "put_call", "shares", "value",
    ]
    assert len(df) == 2
    assert df.iloc[0]["ticker"] == "AAPL"
...
def test_holdings_13f_missing_put_call_fills_nan() -> None:
    ...
    with (
        ...
        result = sec_edgar_holdings_13f(cik="1067983")
    assert "put_call" in result.data.columns
    assert result.data["put_call"].isna().all()
