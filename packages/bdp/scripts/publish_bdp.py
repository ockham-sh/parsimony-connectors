"""Publish parsimony-bdp catalog snapshots.
...
(``~/.cache/parsimony/catalogs/bdp/`` on Linux). Override with
``PARSIMONY_CACHE_DIR`` if you need a different cache root (HF runners,
alternate disks).

Run with ``uv run --extra publish python scripts/publish_bdp.py`` from
the package root. After publishing, push to HF::

    hf upload ockham/bdp "$(parsimony cache info | grep catalogs)/bdp/"
"""
...
TARGET_ROOT = catalogs_dir("bdp")


async def _main() -> int:
    print(f"=== publish bdp start {time.strftime('%Y-%m-%dT%H:%M:%S%z')} ===", flush=True)
