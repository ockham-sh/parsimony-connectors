"""Publish parsimony-boj catalog snapshots.
...
(``~/.cache/parsimony/catalogs/boj/`` on Linux). Override with
``PARSIMONY_CACHE_DIR`` if you need a different cache root (HF runners,
alternate disks).

Run with ``uv run --extra publish python scripts/publish_boj.py`` from
the package root. After publishing, push to HF::

    hf upload ockham/boj "$(parsimony cache info | grep catalogs)/boj/"
"""
...
TARGET_ROOT = catalogs_dir("boj")


async def _main() -> int:
    print(f"=== publish boj start {time.strftime('%Y-%m-%dT%H:%M:%S%z')} ===", flush=True)
