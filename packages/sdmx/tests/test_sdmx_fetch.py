...
class TestSdmxFetch:
    ...
    def test_fetch_result_carries_output_spec(self, patch_sdmx: dict[str, Any]) -> None:
        ...
        # The spec rides the result verbatim: per-flow code columns are covered by
        # the METADATA wildcard, not enumerated into the declaration.
        assert roles["*"] == ColumnRole.METADATA
        assert "FREQ_code" not in roles
        assert "FREQ_code" in result.data.columns
...
class TestSdmxFetchOutput:
    def test_static_schema_roles_and_wildcard(self) -> None:
        ...
        # TIME_PERIOD stays a raw SDMX period string (never coerced to datetime).
