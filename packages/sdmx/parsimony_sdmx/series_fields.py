...
def known_search_fields(dsd_order: tuple[str, ...] | list[str]) -> set[str]:
    """Every real column of a series catalog — the valid ``filter`` keys.
    ...
    against, the ``{dim}_label`` fields are the search surface a connector declares
    — that is a connector policy decision, not a property of the schema.
    """
    fields = {TITLE_FIELD}
