"""Publish parsimony-snb catalog snapshots.
...
(``~/.cache/parsimony/catalogs/snb/`` on Linux). Override with
``PARSIMONY_CACHE_DIR`` if you need a different cache root (HF runners,
alternate disks).

Run with ``uv run --extra publish python scripts/publish_snb.py`` from
the package root. After publishing, push to HF::

    hf upload ockham/snb "$(parsimony cache info | grep catalogs)/snb/"
"""
...
TARGET_ROOT = catalogs_dir("snb")


async def _main() -> int:
    print(f"=== publish snb start {time.strftime('%Y-%m-%dT%H:%M:%S%z')} ===", flush=True)
