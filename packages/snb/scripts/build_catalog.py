...
def build(*, save: str | None, push: str | None) -> None:
    catalog = build_snb_catalog()
    logger.info("Built %s catalog with %d entries", catalog.name, len(catalog))
    if save is not None:
        catalog.save(save, builder="packages/snb/scripts/build_catalog.py")
    if push is not None:
        catalog.save(push, builder="packages/snb/scripts/build_catalog.py")
