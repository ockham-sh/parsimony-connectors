...
from parsimony_polymarket import polymarket_search_events
...
def test_polymarket_search_maps_status(status: int, exc_type: type[ConnectorError]) -> None:
    respx.get(_SEARCH_URL).mock(return_value=httpx.Response(status, text=f"status={status}"))
...
def test_polymarket_provider_error_carries_status_code() -> None:
    respx.get(_SEARCH_URL).mock(return_value=httpx.Response(503, text="unavailable"))
